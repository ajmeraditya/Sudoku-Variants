...
function Diagonal() {
  ...
  function checkDiagonal1(grid, num) {
    for (let i = 0; i < 9; i++) {
      if (grid[i][i] === num) {
        return false; // Number is already present in diagonal1
      }
    }
    return true; // Number is not present in diagonal1
  }

  function checkDiagonal2(grid, num) {
    for (let i = 0; i < 9; i++) {
      if (grid[i][8 - i] === num) {
        return false; // Number is already present in diagonal2
      }
    }
    return true; // Number is not present in diagonal2
  }
  ...
  function checkValid(grid, row, col, num) {
    if (row === 4 && col === 4) {
      if (
        ...
        checkDiagonal1(grid, num) &&
        checkDiagonal2(grid, num)
      ) {
        return true;
      }
    } else if (row === col) {
  ...
  return (
    <div className="App">
      <div className="App-header">
        <h3>Unique Diagonal Sudoku Puzzle</h3>
        <div className=" flex">
          <table>
            <tbody>
              {arr.map((row, rIndex) => {
