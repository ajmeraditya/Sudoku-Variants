...
function Nonconsecutive() {
  ...
  function checkConsecutiveValues(grid, row, col, num) {
    const adjacentCells = [
      [row - 1, col], // Above
      [row + 1, col], // Below
      [row, col - 1], // Left
    ...
    for (let i = 0; i < adjacentCells.length; i++) {
      const [adjRow, adjCol] = adjacentCells[i];
      ...
      if (
        ...
        Math.abs(grid[adjRow][adjCol] - num) === 1
      ) {
        return false; // Adjacent cell contains a consecutive value
      }
    }

    return true; // No adjacent cell contains a consecutive value
  }
  ...
  function checkValid(grid, row, col, num) {
    if (
      ...
      checkConsecutiveValues(grid, row, col, num)
    ) {
      return true;
    }
    return false;
  ...
  return (
    <div className="App">
      <div className="App-header">
        <h3>Non-Consecutive Sudoku Puzzle</h3>
        <div className=" flex">
          <table>
            <tbody>
              {arr.map((row, rIndex) => {
